...
local function EnterApartment(source, apartmentId, aptName)
    ...
    local coords = Vector(Apartments.Locations[aptName].coords[1], Apartments.Locations[aptName].coords[2], Apartments.Locations[aptName].coords[3] + offset)
...
RegisterServerEvent('qb-apartments:server:RingDoor', function(source, data)
    ...
    local Player = exports['qb-core']:GetPlayer(source)
    if not (apartmentId and Player) then
        return
    end
