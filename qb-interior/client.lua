...
exports('qb-interior', 'DespawnInterior_C', function(id)
    if interiors[id] then
        if interiors[id].object then
            interiors[id].object:K2_DestroyActor()
        end
        if interiors[id].light then
            interiors[id].light:K2_DestroyActor()
        end
        interiors[id] = nil
    end
end)
...
local function CreateShell(spawn, exitXYZH, shellData, teleport)
    ...
    if teleport or teleport == nil then
        Timer.SetTimeout(function()
            TriggerServerEvent('qb-interior:server:teleportPlayer', spawn.X - POIOffsets.exit.x, spawn.Y - POIOffsets.exit.y, spawn.Z + POIOffsets.exit.z)
        end, 1000)
    end
