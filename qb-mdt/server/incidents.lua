...
MDT.RegisterRpc('getIncidents', function(source, data)
    ...
    local rows
    if query ~= '' then
        rows = MDT.Select('SELECT id, title, author_name, created, updated FROM mdt_incidents WHERE role = ? AND title LIKE ? ORDER BY created DESC LIMIT ? OFFSET ?', { role, '%' .. query .. '%', Config.PageSize, offset })
    else
        rows = MDT.Select('SELECT id, title, author_name, created, updated FROM mdt_incidents WHERE role = ? ORDER BY created DESC LIMIT ? OFFSET ?', { role, Config.PageSize, offset })
    end
...
MDT.RegisterRpc('saveIncident', function(source, data)
    ...
    if id then
        ...
        MDT.Execute('UPDATE mdt_incidents SET title = ?, details = ?, updated = CURRENT_TIMESTAMP WHERE id = ?', { data.title, details, id })
        MDT.Log(role, Player, 'incident:update', tostring(id))
    else
        MDT.Execute('INSERT INTO mdt_incidents (role, title, details, author_cid, author_name) VALUES (?, ?, ?, ?, ?)', { role, data.title, details, Player.PlayerData.citizenid, MDT.CharName(Player) })
        ...
        local newRows = MDT.Select('SELECT id FROM mdt_incidents WHERE role = ? AND title = ? AND author_cid = ? ORDER BY id DESC LIMIT 1', { role, data.title, Player.PlayerData.citizenid })
        id = newRows and newRows[1] and tonumber(newRows[1].id) or nil
        if not id then
            return { ok = false, message = 'Failed to save report' }
        end
    ...
    if type(data.links) == 'table' then
        ...
        for _, link in ipairs(data.links) do
            if type(link) == 'table' and LinkTypes[link.link_type] and type(link.identifier) == 'string' and link.identifier ~= '' then
                MDT.Execute('INSERT INTO mdt_incident_links (incident_id, link_type, identifier, label) VALUES (?, ?, ?, ?)', { id, link.link_type, link.identifier, tostring(link.label or '') })
            end
        end
    end
...
MDT.RegisterRpc('processArrest', function(source, data)
    ...
    MDT.Execute('INSERT INTO mdt_convictions (citizenid, incident_id, charges, fine, sentence, officer_cid, officer_name) VALUES (?, ?, ?, ?, ?, ?, ?)', { citizenid, incidentId, JSON.stringify(charges), totalFine, totalSentence, Player.PlayerData.citizenid, MDT.CharName(Player) })
    ...
    local convRows = MDT.Select('SELECT id FROM mdt_convictions WHERE citizenid = ? AND officer_cid = ? ORDER BY id DESC LIMIT 1', { citizenid, Player.PlayerData.citizenid })
    ...
    if Target then
        ...
        TriggerClientEvent(Target.PlayerData.source, 'QBCore:Notify', ('You were convicted: $%s fine, %s months'):format(totalFine, totalSentence), 'error')
