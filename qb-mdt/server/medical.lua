...
MDT.RegisterRpc('getMedicalRecords', function(source, data)
    ...
    if not Player or role ~= 'ems' then
        return { ok = false }
    end
    ...
    local rows = MDT.Select('SELECT * FROM mdt_medical WHERE citizenid = ? ORDER BY created DESC LIMIT 50', { citizenid }) or {}
    for i = 1, #rows do
...
MDT.RegisterRpc('saveMedicalRecord', function(source, data)
    ...
    if id then
        MDT.Execute('UPDATE mdt_medical SET injuries = ?, treatment = ?, medications = ?, notes = ?, flags = ? WHERE id = ?', { injuries, treatment, medications, notes, flags, id })
        MDT.Log(role, Player, 'medical:update', tostring(id))
    else
        MDT.Execute('INSERT INTO mdt_medical (citizenid, injuries, treatment, medications, notes, flags, author_cid, author_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', { data.citizenid, injuries, treatment, medications, notes, flags, Player.PlayerData.citizenid, MDT.CharName(Player) })
        id = MDT.LastInsertId()
...
local function AddMedicalRecord(data)
    ...
    MDT.Execute('INSERT INTO mdt_medical (citizenid, injuries, treatment, medications, notes, flags, author_cid, author_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
        data.citizenid,
