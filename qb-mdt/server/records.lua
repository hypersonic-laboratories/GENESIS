...
function MDT.ExpireWarrants()
    MDT.Execute('UPDATE mdt_warrants SET status = \'expired\' WHERE status = \'active\' AND expires_at IS NOT NULL AND datetime(expires_at) <= datetime(\'now\')')
end
...
MDT.RegisterRpc('getWarrants', function(source)
    ...
    MDT.ExpireWarrants()
    local rows = MDT.Select('SELECT * FROM mdt_warrants WHERE status = \'active\' ORDER BY created DESC LIMIT 100')
...
MDT.RegisterRpc('createWarrant', function(source, data)
    ...
    if type(data) ~= 'table' or type(data.citizenid) ~= 'string' or data.citizenid == '' or type(data.reason) ~= 'string' or data.reason == '' then
        return { ok = false, message = 'Citizen and reason are required' }
    end

    local days = tonumber(data.expiresDays)
    if days and days > 0 then
        MDT.Execute('INSERT INTO mdt_warrants (citizenid, incident_id, reason, author_cid, author_name, expires_at) VALUES (?, ?, ?, ?, ?, datetime(\'now\', \'+\' || ? || \' days\'))', { data.citizenid, tonumber(data.incidentId), data.reason, Player.PlayerData.citizenid, MDT.CharName(Player), math.floor(days) })
    else
        MDT.Execute('INSERT INTO mdt_warrants (citizenid, incident_id, reason, author_cid, author_name) VALUES (?, ?, ?, ?, ?)', { data.citizenid, tonumber(data.incidentId), data.reason, Player.PlayerData.citizenid, MDT.CharName(Player) })
    end
...
MDT.RegisterRpc('updateWarrant', function(source, data)
    ...
    if not id or (status ~= 'served' and status ~= 'cancelled') then
        return { ok = false }
    end
...
local function IsWarranted(citizenid)
    MDT.ExpireWarrants()
    local rows = MDT.Select('SELECT id FROM mdt_warrants WHERE citizenid = ? AND status = \'active\' LIMIT 1', { citizenid })
...
MDT.RegisterRpc('getBolos', function(source)
    ...
    local rows = MDT.Select('SELECT * FROM mdt_bolos WHERE role = ? AND status = \'active\' ORDER BY priority ASC, created DESC LIMIT 100', { role })
...
MDT.RegisterRpc('createBolo', function(source, data)
    ...
    if boloType == 'vehicle' then
        identifier = identifier:upper()
    end
    local priority = math.min(3, math.max(1, tonumber(data.priority) or 2))

    MDT.Execute('INSERT INTO mdt_bolos (role, bolo_type, identifier, title, description, priority, image, author_cid, author_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', { role, boloType, identifier, data.title, tostring(data.description or ''), priority, tostring(data.image or ''), Player.PlayerData.citizenid, MDT.CharName(Player) })
    local id = MDT.LastInsertId()
...
local function GetVehicleBolo(plate)
    ...
    local rows = MDT.Select('SELECT * FROM mdt_bolos WHERE status = \'active\' AND bolo_type = \'vehicle\' AND UPPER(identifier) = ? LIMIT 1', { plate:upper() })
